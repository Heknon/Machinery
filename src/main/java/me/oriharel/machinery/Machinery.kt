...
class Machinery : JavaPlugin() {
    ...
    lateinit var machineManager: MachineManager private set
    lateinit var structureManager: StructureManager private set
    lateinit var fuelManager: FuelManager private set
    lateinit var machineDataManager: MachineDataManager private set
    private lateinit var signMenuFactory: SignMenuFactory
    ...
    override fun onEnable() { // save all configuration files
        ...
        structureManager.registerOnDoneCallback {
            ...
            Bukkit.getWorlds().forEach { world: World? -> machineDataManager.loadMachineData(world) }
    ...
    fun createSignInput(target: Player?, response: BiPredicate<Player?, Array<String?>?>?, vararg defaultLines: String?) {
        signMenuFactory
                .newMenu(Lists.newArrayList<String>(*defaultLines))
                .reopenIfFail()
                .response(response)
                .open(target)
    }
    ...
    fun updateMachineBlock(machine: PlayerMachine, fromAsyncThread: Boolean) {
        if (fromAsyncThread) Bukkit.getScheduler().runTask(this, Runnable { machineManager.setPlayerMachineBlock(machine.machineCore.block, machine) }) else machineManager.setPlayerMachineBlock(machine.machineCore.block, machine)
    }
    ...
    private fun setupCommandManager() {
        ...
        ) { machineManager.machines.stream().map { obj: Machine -> obj.machineName }.collect(Collectors.toList()) }
        commandManager.commandContexts.registerIssuerAwareContext<Machine>(Machine::class.java) { c: BukkitCommandExecutionContext ->
    ...
    companion object {
        ...
        lateinit var instance: Machinery private set
    }
}