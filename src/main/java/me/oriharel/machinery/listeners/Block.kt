...
class Block(private val machinery: Machinery) : Listener {
    ...
    fun onBlockPlace(e: BlockPlaceEvent) {
        ...
        if (!machinery.machineManager.buildMachine(e.player.uniqueId, machine, e.block.location)) {
    ...
    fun onBlockBreak(e: BlockBreakEvent) {
        if (machinery.machineManager.temporaryPreRegisterMachineLocations.contains(e.block.location) || machinery.machineManager.machinePartLocations.contains(e.block.location)) {
    ...
    fun onEntityExplosion(e: EntityExplodeEvent) {
        ...
        for (block in blocks) {
            if (machinery.machineManager.temporaryPreRegisterMachineLocations.contains(block.location) || machinery.machineManager.machinePartLocations.contains(block.location)) {
                e.isCancelled = true
            }
        }
    }
