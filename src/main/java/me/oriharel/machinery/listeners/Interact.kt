...
class Interact(private val machinery: Machinery) : Listener {
    ...
    fun onPlayerInteract(e: PlayerInteractEvent) {
        ...
        if (machinery.machineManager.machinePartLocations.contains(e.clickedBlock!!.location)) {
            e.setUseInteractedBlock(Event.Result.DENY)
        }
        val machine = machinery.machineManager.machineCores[e.clickedBlock!!.location]
        if (machine != null) {
            if (!machine.playersWithAccessPermission.contains(e.player.uniqueId)) {
