...
object Utils {
     ...
     */
    fun getLocationPlaceholders(location: Location?, vararg extraPlaceholders: Placeholder?): List<Placeholder> {
        val placeholders: MutableList<Placeholder> = if (extraPlaceholders.size == 0) ArrayList() else LinkedList(Arrays.asList(*extraPlaceholders))
     ...
     * @return the generated placeholder for location + extra placeholders given
     */
    fun getLocationPlaceholders(location: Location?, extraPlaceholders: List<Placeholder?>?): List<Placeholder> {
     ...
     */
    fun getMachinePlaceholders(machine: PlayerMachine, extraPlaceholders: List<Placeholder?>?): List<Placeholder> {
     ...
     */
    fun getAmountThingPlaceholder(amount: Int, thing: String?, extraPlaceholders: List<Placeholder?>): List<Placeholder> {
     ...
     * @return the generated placeholder for machine + extra placeholders given
     */
    fun getMachinePlaceholders(machine: PlayerMachine, vararg extraPlaceholders: Placeholder?): List<Placeholder> {
        val placeholders: MutableList<Placeholder> = if (extraPlaceholders.size == 0) ArrayList() else LinkedList(Arrays.asList(*extraPlaceholders))
        ...
        placeholders.add(Placeholder("%upgrade_loot_bonus_name%",
                machine.upgrades.stream().filter { u: AbstractUpgrade? -> u.getUpgradeType() == UpgradeType.LOOT_BONUS }.findAny().get().upgradeName))
        placeholders.add(Placeholder("%upgrade_loot_bonus_level%",
                machine.upgrades.stream().filter { u: AbstractUpgrade? -> u.getUpgradeType() == UpgradeType.LOOT_BONUS }.findAny().get().level))
        placeholders.add(Placeholder("%upgrade_speed_name%",
                machine.upgrades.stream().filter { u: AbstractUpgrade? -> u.getUpgradeType() == UpgradeType.SPEED }.findAny().get().upgradeName))
        placeholders.add(Placeholder("%upgrade_speed_level%",
                machine.upgrades.stream().filter { u: AbstractUpgrade? -> u.getUpgradeType() == UpgradeType.SPEED }.findAny().get().level))
        return placeholders
    }
     ...
     * @return the generated placeholder for amount thing + extra placeholders given
     */
    fun getAmountThingPlaceholder(amount: Int, thing: String?, vararg extraPlaceholders: Placeholder?): List<Placeholder> {
        val placeholders: MutableList<Placeholder> = if (extraPlaceholders.size == 0) ArrayList() else LinkedList(Arrays.asList(*extraPlaceholders))
