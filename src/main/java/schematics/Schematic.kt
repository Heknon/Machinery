 ...
 * @version 2.0.3
 */
class Schematic(private val plugin: JavaPlugin, val schematic: File) {
    var width: Short = 0
        private set
    var height: Short = 0
        private set
    ...
    val delayedBlocks: List<Material>
    ...
    fun pasteSchematic(loc: Location,
                       paster: Player?,
                       time: Int,
                       callback: CallbackP<List<Location?>?>,
                       vararg option: Options?): List<Location?>? {
        try {
            ...
            val options: List<Options> = Arrays.asList(*option)
            val tracker = Data()
...
//            validData.add(Material.SANDSTONE_WALL);
     ...
     */
    @Throws(SchematicNotLoadedException::class)
    fun pasteSchematic(location: Location, paster: Player?, callback: CallbackP<List<Location?>?>, vararg options: Options?): List<Location?>? {
    ...
    fun loadSchematic(): Schematic {
        try {
             ...
             * Load NBT data
             */if (tiles != null) {
                tiles.forEach(Consumer { a: NBTBase? ->
    ...
    fun getSigns(): Map<Vector, List<String?>> {
        return signs
    }

    fun getBlocks(): Map<Int, BlockData> {
        return blocks
    }
     ...
     */
    init {
        delayedBlocks = Arrays.asList(Material.LAVA,
                Material.WATER,
                Material.GRASS,
