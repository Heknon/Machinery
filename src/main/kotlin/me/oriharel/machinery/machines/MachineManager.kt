...
class MachineManager(private val machinery: Machinery) {
    ...
    private fun registerNewPlayerMachine(playerMachine: PlayerMachine?, machineLocations: Set<Location?>) {
        try {
            setPlayerMachineBlock(playerMachine?.core?.block!!, playerMachine)
            val locations: Array<Location?> = machineLocations.toTypedArray()
            setPlayerMachineLocations(playerMachine.core.block, Arrays.copyOf(locations, locations.size, Array<Location>::class.java))
            machineCores[playerMachine.core] = playerMachine
            machinePartLocations.addAll(machineLocations)
            val machineLocation = playerMachine.core
    ...
    fun unregisterPlayerMachine(machine: PlayerMachine) {
        val playerMachinePartLocations = getPlayerMachineLocations(machine.core?.block!!)

        machineCores.remove(machine.core)
        machinePartLocations.removeAll(playerMachinePartLocations)
        machinery.machineDataManager?.removeMachineCoreLocation(machine.core)
    }
    ...
    fun <T : Machine?> buildMachine(playerUuid: UUID, machine: T, buildLocation: Location): Boolean {
        return try {
            ...
            val locations = machine!!.structure!!.build(buildLocation, p, machine.coreBlockType) { printResult: PrintResult? ->
