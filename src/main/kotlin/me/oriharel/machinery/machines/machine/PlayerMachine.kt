...
class PlayerMachine(maxFuel: Int, fuelDeficiency: Int,
                    structure: Structure?, recipe: CustomRecipe<*>?, name: String?, coreBlockType: Material?,
                    val playersWithAccessPermission: Set<UUID?>?, var totalResourcesGained: Double, var resourcesGained: Map<Material?, ItemStack?>?, var energyInMachine: Int, val core: Location?, var zenCoinsGained: Double,
                    var totalZenCoinsGained: Double, val owner: UUID?, @field:JsonAdapter(AbstractUpgradeTypeAdapter::class) val upgrades: List<AbstractUpgrade?>?, @field:Transient val factory: MachineFactory) : Machine(maxFuel, fuelDeficiency, structure, recipe, name, coreBlockType, factory) {
    ...
    fun deconstruct(): MachineItem {
        val machineManager: MachineManager = Machinery.instance?.machineManager!!
        machineManager.clearMachineTileStateDataFromBlock(core!!.block)
        val machinePartLocations = machineManager.getPlayerMachineLocations(core.block)
