...
import me.oriharel.machinery.machines.MachineInventoryImpl
    ...
    fun onPlayerInteract(e: PlayerInteractEvent) {
        ...
        if (machine != null) {
            if (machine.playersWithAccessPermission?.contains(e.player.uniqueId) == false) {
                Message("messages.yml", "open_attempt_no_access", e.player, Utils.getLocationPlaceholders(machine.core,
            ...
            e.setUseInteractedBlock(Event.Result.DENY)
            e.isCancelled = true
            Message("messages.yml", "open_machine_gui", e.player, Utils.getLocationPlaceholders(machine.core,
                    Utils.getMachinePlaceholders(machine))).send()
            e.player.closeInventory()
