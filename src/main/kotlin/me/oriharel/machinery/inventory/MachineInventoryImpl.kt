...
class MachineInventoryImpl(private val machine: PlayerMachine, private val p: Player, private val machinery: Machinery) {
    ...
    private fun handleZenCoinWithdrawalSign(p: Player, lines: Array<String>): Boolean {
        Message("messages.yml", "sign_input.withdraw", p, Utils.getLocationPlaceholders(machine.machineCore,
        ...
        try {
            val num = validateAndGetNumber(lines, machine.zenCoinsGained as Int, "Zen Coins",
                    Message("messages.yml", "sign_input.not_enough_resources_in_machine", p, Utils.getLocationPlaceholders(machine.machineCore,
    ...
    private fun handleResourceWithdrawalSign(p: Player, lines: Array<String>, resource: ItemStack): Boolean {
        ...
        if (!Utils.inventoryHasSpaceForItemAdd(p.inventory)) {
            Message("messages.yml", "not_enough_inventory_space", p, Utils.getLocationPlaceholders(machine.machineCore, Utils.getMachinePlaceholders(machine,
    ...
    private fun validateAndGetNumber(lines: Array<String>, maxCanTakeAmount: Int, name: String, aboveMaxErrorMessage: Message, mutatePlaceholder: Boolean = true): Int {
        return try {
            ...
            if (num < 0) {
                Message("messages.yml", "sign_input.entered_number_below_zero", p, Utils.getLocationPlaceholders(machine.machineCore,
            ...
            Message("messages.yml", "sign_input.invalid_number", p, Utils.getLocationPlaceholders(machine.machineCore, Utils.getMachinePlaceholders(machine))).send()
            -1
        }
    }
